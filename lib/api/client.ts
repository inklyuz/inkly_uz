...
if (!rawBaseUrl && process.env.NODE_ENV === "production") {
  throw new Error("NEXT_PUBLIC_API_URL production muhitida majburiy")
}

const BASE_URL = (rawBaseUrl ?? "http://127.0.0.1:8000/api/v1").replace(/\/$/, "")
...
try {
  ...
  throw new Error("NEXT_PUBLIC_API_URL noto'g'ri URL")
}
...
const MEDIA_CDN_URL = (
  ...
  `${parsedBaseUrl.origin}/uploads`
).replace(/\/+$/, "")

let mediaBaseUrl: URL

try {
  mediaBaseUrl = new URL(MEDIA_CDN_URL)

  if (!["http:", "https:"].includes(mediaBaseUrl.protocol)) {
    throw new Error("Unsupported media protocol")
  }
} catch {
  throw new Error(
    "NEXT_PUBLIC_MEDIA_CDN_URL noto'g'ri URL",
  )
}
...
export function getMediaUrl(
  ...
  // Blob URL — browser temporary files
  if (value.startsWith("blob:")) {
    return value
  }

  // Data URL'larni qabul qilmaymiz
  ...
  if (/^https?:\/\//i.test(value)) {
    try {
      ...
      if (
        url.protocol === "https:" &&
        (
          url.hostname === mediaBaseUrl.hostname ||
          url.hostname === parsedBaseUrl.hostname
        )
      ) {
        return url.toString()
      }
  ...
  // Protocol-relative URL'ni bloklaymiz
  if (value.startsWith("//")) {
    return null
  }
  ...
  // Agar DB'da xato tarzda uploads/ saqlangan bo'lsa,
  // double /uploads/ bo'lishining oldini olamiz.
...
export async function apiRequest<T>(path: string, options: RequestOptions = {}): Promise<T> {
  ...
  // API callers must provide a relative path. This prevents an accidental
  // absolute URL from bypassing the configured API origin.
  if (
    !path.startsWith("/") ||
    path.startsWith("//") ||
    /^[a-z][a-z0-9+.-]*:/i.test(path)
  ) {
    throw new ApiRequestError("INVALID_API_PATH", "API yo'li noto'g'ri")
  }
