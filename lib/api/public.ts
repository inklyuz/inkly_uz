 ...
 *   GET /public/posts/:post_id                 → PostResponse
 *   GET /public/posts/:year/:month/:day/:ref   → PostResponse
 *   GET /public/posts/:author_slug/:post_slug  → PostResponse (legacy)
 *   GET /public/posts/:slug                    → 301 canonical redirect (legacy)
...
export const publicPostsApi = {
  ...
  /**
   * GET /public/posts/:year/:month/:day/:id-:slug
   * Canonical public URL contract; backend postni id orqali topadi.
   */
  getByCanonicalRef: (
    year: string,
    month: string,
    ...
    apiRequest<PostResponse>(
      `/public/posts/${encodeURIComponent(year)}/${encodeURIComponent(month)}/${encodeURIComponent(day)}/${encodeURIComponent(postRef)}`,
      { token },
    ),
...
const _getPostByCanonicalRefCached = cache(
  (year: string, month: string, day: string, postRef: string, token?: string) =>
    publicPostsApi.getByCanonicalRef(year, month, day, postRef, token)
)
...
/** GET /public/posts/:year/:month/:day/:id-:slug — canonical post route */
export const getPublicPostByCanonicalSafe = createSafeItemWrapper(
  (year: string, month: string, day: string, postRef: string, token?: string) =>
    _getPostByCanonicalRefCached(year, month, day, postRef, token),
  { errorPrefix: "PUBLIC_POST_CANONICAL" },
)
