...
import type { CategoryPublicResponse, CategoryResponse, Page, PostListItem } from "@/types/api"
...
export const categoriesApi = {
  ...
  adminList: (token: string) =>
    apiRequest<CategoryResponse[]>("/categories/admin", { token }),

  create: (token: string, data: CategoryCreateData) =>
    apiRequest<CategoryResponse>("/categories", { method: "POST", body: data, token }),
  ...
  update: (token: string, uuid: string, data: CategoryUpdateData) =>
    apiRequest<CategoryResponse>(`/categories/${encodeURIComponent(uuid)}`, {
      method: "PATCH",
      body: data,
      token,
    }),
