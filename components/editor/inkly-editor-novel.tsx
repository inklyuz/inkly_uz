...
const ImageNode = Node.create({
  ...
  parseHTML() {
    return [
      {
        // Yangi format — <figure class="inkly-figure"><img/><figcaption/></figure>
        tag: "figure.inkly-figure",
        getAttrs: (el) => {
      ...
      {
        // Eski maqolalar — oddiy <img class="inkly-image"> (izohsiz)
        tag: "img.inkly-image",
        getAttrs: (el) => {
          if (!(el instanceof HTMLElement)) return false
          return { src: el.getAttribute("src"), alt: el.getAttribute("alt"), caption: "" }
        },
      },
    ]
  },
  renderHTML({ HTMLAttributes }) {
    ...
    const children: unknown[] = [["img", { src: src ?? "", alt: alt ?? "", class: "inkly-image" }]]
    if (caption) children.push(["figcaption", { class: "inkly-image-caption" }, caption])
    return ["figure", { class: "inkly-figure" }, ...children]
  },
  addNodeView() {
    return ({ node, editor, getPos }) => {
      const figure = document.createElement("figure")
      figure.className = "inkly-figure"

      const img = document.createElement("img")
      img.className = "inkly-image"
      img.src = node.attrs.src ?? ""
      img.alt = node.attrs.alt ?? ""
      figure.appendChild(img)

      const figcaption = document.createElement("figcaption")
      figcaption.className = "inkly-image-caption"
      figcaption.contentEditable = editor.isEditable ? "true" : "false"
      figcaption.setAttribute("data-placeholder", "Rasmga izoh qo'shing (ixtiyoriy)")
      ...
      const commit = () => {
        ...
        const tr = editor.state.tr.setNodeMarkup(pos, undefined, { ...current.attrs, caption: text })
        editor.view.dispatch(tr)
      }

      figcaption.addEventListener("blur", commit)
      figcaption.addEventListener("keydown", (e) => {
        e.stopPropagation()
        if (e.key === "Enter") { e.preventDefault(); figcaption.blur() }
      })
      ...
      return {
        dom: figure,
        update(updatedNode) {
          if (updatedNode.type.name !== "inklyImage") return false
          if (img.src !== updatedNode.attrs.src) img.src = updatedNode.attrs.src ?? ""
          if (document.activeElement !== figcaption) {
            const next = updatedNode.attrs.caption ?? ""
            if (figcaption.textContent !== next) figcaption.textContent = next
          }
          return true
        },
        stopEvent(event) {
          return event.target instanceof globalThis.Node && figcaption.contains(event.target)
        },
      }
    }
  },
      ...
      try {
        ...
        requestAnimationFrame(() => {
          editorInstanceRef.current
            ?.chain()
            .focus()
            .insertContent({ type: "inklyImage", attrs: { src: upload.url, caption: "" } })
            .run()
