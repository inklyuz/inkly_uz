...
// Bu sahifalarda Navbar barcha ekran o'lchamlarida to'liq yashiriladi.
const FULLY_HIDDEN_PAGES: RegExp[] = [
  /^\/login$/,
  /^\/register(\/.*)?$/,
  /^\/@[^/]+$/,
  /^\/@[^/]+\/[^/]+$/,
...
export function Navbar() {
  ...
  const isFullyHidden = FULLY_HIDDEN_PAGES.some((pattern) => pattern.test(pathname))
    ...
    >
      <div className="mx-auto flex h-[76px] w-full max-w-[1400px] items-center px-6 sm:px-8 lg:px-10">
        ...
        <div className="ml-auto hidden items-center gap-4 lg:flex">
          ...
          {/* ── Auth holati ──────────────────────────────────────────────────
              loading: true  → token bor, server dan tasdiqlanyapti → skeleton
              loading: false + user → avatar + "Maqola yozish"
              loading: false + !user → "Kirish" + "Boshlash"
              
              getInitialLoading() tufayli login qilmagan foydalanuvchilar uchun
              loading hech qachon true bo'lmaydi — skeleton ko'rinmaydi.
          ──────────────────────────────────────────────────────────────────── */}
          {loading ? (
            <div className="h-9 w-[130px] animate-pulse rounded-full bg-bg-muted" aria-hidden />
          ) : user ? (
            <>
              <Link href="/write">
