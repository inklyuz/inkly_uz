    ...
    return (
      ...
      >
        <div className="flex flex-col p-4">
          ...
          {post.excerpt && (
            <p className="mt-2 line-clamp-2 text-[12.5px] leading-[1.6] text-text-secondary">
              {post.excerpt}
            </p>
          )}
          ...
          <div className="mt-4 flex items-center justify-between gap-3 border-t border-border-default pt-3">
            <Link href={`/@${post.author.username}`} className="flex min-w-0 items-center gap-2">
              ...
              <span className="truncate text-[11px] font-medium text-text-primary">@{post.author.username}</span>
  ...
  return (
    <article className="group relative flex overflow-hidden rounded-card border border-border-default bg-white transition-all duration-200 hover:border-[#DCD5CC] hover:shadow-[0_6px_20px_rgba(20,20,20,0.05)]">
      ...
      <div className="flex min-w-0 flex-1 flex-col px-5 py-4">
        ...
        {post.excerpt && (
          <p className="mt-1.5 line-clamp-2 max-w-[650px] text-[13px] leading-[1.65] text-text-secondary">
            {post.excerpt}
          </p>
        )}
        ...
        <div className="mt-4 flex flex-wrap items-end justify-between gap-3 border-t border-border-default pt-3">
          <div className="flex min-w-0 flex-wrap items-center gap-x-3 gap-y-1">
            <Link href={`/@${post.author.username}`} className="flex min-w-0 items-center gap-2">
              <Avatar src={post.author.avatar} name={post.author.full_name} size={24} />
              <span className="max-w-[120px] truncate text-[11px] font-medium text-text-primary transition-colors hover:text-primary">
            ...
            {readTime && (
              <>
                <span className="h-3 w-px shrink-0 bg-border-default" />
                <span className="flex items-center gap-1 whitespace-nowrap text-[10px] text-text-muted">
                  <Clock size={10} />
                  {readTime}
                </span>
              </>
          ...
          <div className="flex shrink-0 items-center gap-3 text-text-muted">
            ...
            <button
              ...
              className={`ml-1 flex h-9 w-9 items-center justify-center rounded-lg transition-colors duration-150 disabled:opacity-50 ${bookmarked
