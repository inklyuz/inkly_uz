  ...
  useEffect(() => {
    ...
    const handleAuth = async (userData: TelegramAuthData) => {
      ...
      try {
        ...
        // Foydalanuvchi ma'lumotlarini olish (username uchun — profile redirect)
        ...
        try {
          ...
          // me() muvaffaqiyatsiz bo'lsa — username'siz davom etamiz
        }
        ...
        onSuccessRef.current(profileUsername)
      } catch (err) {
        if (!mountedRef.current) return
        handledRef.current = false
        setError(
    ...
    /*
     * MUHIM: BotFather'dagi username aynan shu bo'lishi kerak (@ belgisisiz).
     * Masalan: @inkly_uz_bot => inkly_uz_bot
     */
    ...
    /*
     * Telegram widget script yuklanmasa fallbackga o'tamiz.
     */
    script.onerror = () => {
      fireError()
    }
    ...
    /*
     * MUHIM: Telegram widget iframe cross-origin.
     * Callback kelmasa fallback ishlaydi (3.5s timeout).
     */
