...
import { checkUsernameSafe } from "@/lib/api/users"
...
export function RegisterStep1({ onNext, onTelegramSuccess }: Step1Props) {
  ...
  // ── Real-time username mavjudligini tekshirish ────────────────────────────
  // Backend /users/check shu maqsadda qurilgan, lekin avval frontend uni
  // hech qayerda chaqirmagan edi — foydalanuvchi faqat submit qilgandan
  // keyin username band ekanini bilar edi.
  const [availability, setAvailability] = useState<"idle" | "checking" | "available" | "taken">("idle")
  const requestIdRef = useRef(0)
  const isUsernameValid = /^[a-zA-Z0-9_]{3,60}$/.test(username)
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      const result = await checkUsernameSafe(username)
      if (requestId !== requestIdRef.current) return // eskirgan javob
      setAvailability(result?.available ? "available" : result ? "taken" : "idle")
    }, 400)
      ...
      <form onSubmit={handleSubmit(onNext)} className="flex flex-col gap-4" noValidate>
        <div className="flex flex-col gap-1.5">
          ...
          <StyledInput
            ...
            hasError={!!errors.username || availability === "taken"}
          ...
          {errors.username ? (
            ...
            <p id="username-error" className="text-xs flex items-center gap-1.5" style={{ color: "var(--color-warning)" }} role="alert">
              <AlertIcon size={12} />Bu username band, boshqasini tanlang
            </p>
          ) : (
            <div id="username-preview" className="flex flex-col gap-1">
              <UsernamePreview username={username} />
              {availability === "checking" && (
                <span className="text-xs" style={{ color: "var(--color-text-muted)" }}>Tekshirilmoqda...</span>
              )}
              {availability === "available" && (
                <span className="text-xs font-medium" style={{ color: "#16a34a" }}>✓ Bu username bo'sh</span>
              )}
            </div>
          )}
        </div>

        <SubmitButton label="Davom etish" loading={false} disabled={availability === "taken"} />
      </form>
