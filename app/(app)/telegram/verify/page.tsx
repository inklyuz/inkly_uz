...
import { Loader2, ShieldCheck, ShieldAlert, RefreshCw, CheckCircle2, ArrowRight, Copy, AlertCircle } from "lucide-react"
...
export default function TelegramVerifyPage() {
  ...
  useEffect(() => {
    ...
    const checkStatus = async () => {
      try {
        const res = await telegramApi.verificationStatus(token)
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-[#141414]">Kanalni tasdiqlash</h1>
          ...
          return (
            <div key={label} className="flex items-center gap-2">
              <div className={cn(
                ...
                isDone ? "bg-green-500 text-white" : isActive ? "bg-[#FF6A00] text-white" : "bg-[#E8E3DD] text-[#6B7280]"
              )}>
                {isDone ? <CheckCircle2 size={16} /> : idx + 1}
              </div>
              {idx < 2 && (
                <ArrowRight size={14} className={isDone ? "text-green-500" : "text-[#E8E3DD]"} />
              )}
            </div>
          )
        })}
      ...
      <div className="rounded-2xl border border-[#E8E3DD] bg-white p-6">
        {step === "idle" && (
          <div className="text-center py-8">
            <div className="mx-auto h-16 w-16 rounded-full bg-[#FFF3E8] flex items-center justify-center mb-4">
              <ShieldCheck size={28} className="text-[#FF6A00]" />
            </div>
            <h3 className="font-semibold text-[#141414] mb-2">Tasdiqlashni boshlang</h3>
            ...
            <Button
              ...
              className="rounded-full bg-[#FF6A00] px-6 py-2 font-semibold text-white hover:bg-[#E85F00]"
            >
              {startLoading ? (
                <>
        ...
        {step === "started" && (
          <div className="text-center py-8">
            ...
            <h3 className="font-semibold text-[#141414] mb-2">Tasdiqlash kutilmoqda...</h3>
            <p className="text-sm text-[#6B7280] mb-6 max-w-md mx-auto">
              Telegram botga o'ting va tasdiqlash kodini yuboring. Status avtomatik tekshiriladi.
            </p>
            {verification && (
              <div className="bg-[#F8F9FB] rounded-xl p-4 max-w-sm mx-auto mb-4">
                <p className="text-xs text-[#6B7280] mb-1">Tasdiqlash tokeni:</p>
                <div className="flex items-center gap-2">
                  <code className="text-xs font-mono bg-white px-2 py-1 rounded flex-1 truncate">{verification.token}</code>
                  <Button variant="ghost" size="sm" onClick={copyToken} className="p-1.5 text-[#6B7280] hover:text-[#141414]">
                    <Copy size={14} />
                  </Button>
                </div>
              </div>
        ...
        {step === "verified" && (
          <div className="text-center py-8">
            ...
            <h3 className="font-semibold text-[#141414] mb-2">Tasdiqlandi!</h3>
            <p className="text-sm text-[#6B7280] mb-6 max-w-md mx-auto">
            ...
            <div className="flex items-center justify-center gap-3">
              ...
              <Button
                ...
                className="rounded-full text-[#36565F] hover:bg-[#F2F4F7]"
              >
                Kanallar
              </Button>
            </div>
        ...
        {step === "failed" && (
          <div className="text-center py-8">
            ...
            <h3 className="font-semibold text-[#141414] mb-2">Tasdiqlash amalga oshmadi</h3>
            <p className="text-sm text-[#6B7280] mb-6 max-w-md mx-auto">
