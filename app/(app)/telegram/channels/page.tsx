...
import { Plus, Trash2, Send, CheckCircle2, ExternalLink, X, RefreshCw } from "lucide-react"
...
export default function TelegramChannelsPage() {
  ...
  const [publishChannel, setPublishChannel] = useState<TelegramChannelResponse | { uuid: "all"; title: string } | null>(null)
  ...
  // Kanallarni va bot havolasini yuklash
  useEffect(() => {
    if (!token) return
    setLoading(true)
    
    Promise.all([
      telegramApi.listChannels(token).then((data) => setChannels(data.items)),
      telegramApi.getBotLink(token).then((data) => {
        if (data && typeof data === "object" && "link" in data) {
          setBotLink(data.link as string)
        }
      }).catch(console.error)
    ]).catch((err) => {
  ...
  const handleToggleAutoPublish = async (channel: TelegramChannelResponse) => {
    if (!token) return
    const next = !channel.auto_publish
    setTogglingAutoPublish(channel.uuid)
    try {
      const updated = await telegramApi.updateChannelSettings(token, channel.uuid, next)
      setChannels((prev) => prev.map((c) => (c.uuid === channel.uuid ? updated : c)))
      toast.success(next ? "Avtomatik nashr yoqildi" : "Avtomatik nashr o'chirildi")
    } catch (err) {
      console.error("Toggle auto_publish failed:", err)
      toast.error("Sozlamani o'zgartirishda xatolik")
    } finally {
      setTogglingAutoPublish(null)
    }
  }

  const handleSyncChannel = async (channelUuid: string) => {
    if (!token) return
    setSyncing(channelUuid)
    try {
      const updated = await telegramApi.syncChannel(token, channelUuid)
      setChannels((prev) => prev.map((c) => (c.uuid === channelUuid ? updated : c)))
  ...
  const handlePublishToAllChannels = async (postUuid: string) => {
    if (!token) return
    setPublishing("all")
    try {
      await telegramApi.publishToAllChannels(token, postUuid)
      toast.success("Maqola barcha tasdiqlangan kanallarga yuborildi")
      setPublishChannel(null)
      setSelectedPost(null)
    } catch (err) {
  ...
  const openPublishModal = async (channel: TelegramChannelResponse | { uuid: "all"; title: string }) => {
    if ("is_verified" in channel && !channel.is_verified) {
  ...
  const handleOpenPublications = (channelUuid: string) => { router.push(`/telegram/channels/${channelUuid}/publications`) }
      ...
      <div className="rounded-2xl border border-border-default bg-white">
        {channels.length > 0 && (
          <div className="flex items-center justify-between border-b border-border-default px-5 py-4">
            <h3 className="font-semibold text-text-primary">Mening kanallarim</h3>
            <Button
              variant="outline"
              size="sm"
              className="text-primary border-primary hover:bg-inkly-orange-light"
              onClick={() => openPublishModal({ uuid: "all", title: "Barcha tasdiqlangan kanallar" })}
            >
            ...
            {channels.map((channel) => (
              ...
              >
                ...
                <div className="flex items-center gap-1 shrink-0">
                  <button
                    type="button"
                    onClick={() => void handleToggleAutoPublish(channel)}
                    disabled={togglingAutoPublish === channel.uuid}
                    title={channel.auto_publish ? "Avtomatik nashr yoqilgan" : "Avtomatik nashr o'chirilgan (faqat qo'lda)"}
                    className={`mr-1 flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-xs font-medium transition-colors disabled:opacity-50 ${
                      channel.auto_publish
                        ? "border-primary/30 bg-inkly-orange-light text-primary"
                        : "border-border-default text-text-muted hover:bg-bg-muted"
                    }`}
                  >
                    {togglingAutoPublish === channel.uuid ? (
                      <LoadingDots size="sm" />
                    ) : (
                      <>
                        <span
                          className={`h-3.5 w-6 shrink-0 rounded-full transition-colors relative ${
                            channel.auto_publish ? "bg-primary" : "bg-border-default"
                          }`}
                        >
                          <span
                            className={`absolute top-0.5 h-2.5 w-2.5 rounded-full bg-white transition-transform ${
                              channel.auto_publish ? "translate-x-3" : "translate-x-0.5"
                            }`}
                          />
                        </span>
                        {channel.auto_publish ? "Avto" : "Qo'lda"}
                      </>
                    )}
                  </button>
                  <Button
                    variant="ghost"
                    size="icon"
                    onClick={() => handleSyncChannel(channel.uuid)}
                    disabled={syncing === channel.uuid}
                    className="text-text-muted hover:text-text-primary hover:bg-bg-muted disabled:opacity-50"
                    title="Yangilash"
                  >
                    {syncing === channel.uuid ? <LoadingDots size="md" /> : <RefreshCw size={16} />}
                  </Button>
                  <Button
                    variant="ghost"
                    size="icon"
      ...
      <div className="rounded-2xl bg-inkly-orange-light border border-inkly-peach p-6">
        ...
        <ol className="space-y-2 text-sm text-text-secondary list-decimal list-inside">
          ...
          <li>
            ...
            ) kanalga admin qiling (Xabar yuborish huquqi bilan)
          </li>
          ...
          >
            ...
            <div className="mt-4 flex justify-end gap-2">
              ...
              <Button
                ...
                onClick={() => {
                  if (publishChannel.uuid === "all") {
                    void handlePublishToAllChannels(selectedPost!.uuid)
                  } else {
                    void handlePublishToChannel(publishChannel.uuid, selectedPost!.uuid)
                  }
                }}
