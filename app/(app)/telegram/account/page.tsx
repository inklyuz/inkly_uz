...
import { Loader2, AlertCircle, CheckCircle2, X, Unlink2, ShieldCheck } from "lucide-react"
...
export default function TelegramAccountPage() {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-[#141414]">Telegram akkaunti</h1>
        ...
        {loading ? (
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              <div className="relative h-16 w-16 shrink-0 rounded-xl bg-[#FFF3E8] flex items-center justify-center">
                <svg width="28" height="28" viewBox="0 0 24 24" fill="currentColor" className="text-[#FF6A00]">
              ...
              <div>
                <p className="font-semibold text-[#141414]">{account.first_name} {account.last_name ?? ""}</p>
                <p className="text-sm text-[#6B7280]">@{account.telegram_username ?? "username yo'q"}</p>
                <p className="text-xs text-[#6B7280] mt-1">ID: {account.telegram_user_id}</p>
              </div>
            </div>
          ...
          <div className="text-center py-12">
            <div className="mx-auto h-16 w-16 rounded-full bg-[#FFF3E8] flex items-center justify-center mb-4">
              <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="text-[#FF6A00]">
                <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z" />
              </svg>
            </div>
            <h3 className="font-semibold text-[#141414]">Telegram akkaunti bog'lanmagan</h3>
            ...
            <Button
              ...
              className="mt-4 rounded-full bg-[#FF6A00] px-6 py-2 font-semibold text-white hover:bg-[#E85F00]"
            >
      ...
      {account && (
        <>
          ...
          <div className="rounded-2xl border border-[#E8E3DD] bg-white p-6">
            <h3 className="font-semibold text-[#141414] mb-4">Amallar</h3>
            <div className="flex flex-col sm:flex-row gap-3">
              <Button
                variant="outline"
                onClick={handleStartVerification}
                className="flex-1 gap-2 rounded-full border-[#E8E3DD] text-[#36565F] hover:border-[#FF6A00] hover:text-[#FF6A00]"
              >
                <ShieldCheck size={16} />
          ...
          {/* Info */}
          <div className="rounded-2xl border border-[#E8E3DD] bg-white p-6">
            <h3 className="font-semibold text-[#141414] mb-4">Ma'lumot</h3>
            <dl className="space-y-3 text-sm">
              <div className="flex justify-between">
                <dt className="text-[#6B7280]">Bog'langan sana</dt>
                <dd className="font-medium text-[#141414]">{new Date(account.created_at).toLocaleDateString("uz-UZ")}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-[#6B7280]">Telegram ID</dt>
                <dd className="font-medium text-[#141414] font-mono">{account.telegram_user_id}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-[#6B7280]">Username</dt>
                <dd className="font-medium text-[#141414]">@{account.telegram_username ?? "—"}</dd>
              </div>
            </dl>
          </div>
          ...
          <div className="rounded-2xl bg-[#FFF3E8] border border-[#FFE8D0] p-6">
            ...
            <ul className="space-y-2 text-sm text-[#36565F]">
              <li className="flex items-center gap-2"><span className="h-1.5 w-1.5 rounded-full bg-[#FF6A00]" /> Maqolalarni Telegram kanallariga avtomatik yuborish</li>
              <li className="flex items-center gap-2"><span className="h-1.5 w-1.5 rounded-full bg-[#FF6A00]" /> Bir nechta kanal qo'shish va ularni boshqarish</li>
              <li className="flex items-center gap-2"><span className="h-1.5 w-1.5 rounded-full bg-[#FF6A00]" /> Kanal tasdiqlash uchun tekshiruv o'tkazish</li>
              <li className="flex items-center gap-2"><span className="h-1.5 w-1.5 rounded-full bg-[#FF6A00]" /> Yuborilgan maqolalar tarixi va holatini kuzatish</li>
            </ul>
          </div>
        </>
      )}
