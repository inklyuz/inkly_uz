    ...
    <div className="space-y-6">
      <div className="rounded-2xl border border-[#E8E3DD] bg-white">
        {privacySettings.map((s, i) => (
          <div
            key={s.key}
            className={`px-6 py-5 ${i < privacySettings.length - 1 ? "border-b border-[#E8E3DD]" : ""}`}
          >
            <p className="text-sm font-medium text-[#141414]">{s.label}</p>
            <p className="mb-3 text-xs text-[#6B7280]">{s.description}</p>
                ...
                <button
                  ...
                  className={`rounded-full border px-4 py-1.5 text-sm font-medium transition-colors ${
                    values[s.key] === opt.value
                      ? "border-[#FF6A00] bg-[#FFF3E8] text-[#FF6A00]"
                      : "border-[#E8E3DD] text-[#36565F] hover:border-[#FF6A00] hover:text-[#FF6A00]"
                  }`}
                >
                  {opt.label}
                </button>
