...
function CoverZone({
  previewUrl, uploading, inputRef, onRemove,
}: {
  ...
  if (previewUrl) {
    return (
      <div className="group relative mb-6 rounded-xl overflow-hidden bg-muted" style={{ aspectRatio: "2.4/1" }}>
        <Image
          src={previewUrl}
          alt="Muqova"
          fill
...
function SidebarPanel({
  ...
  customSlug: string | null; onSlug: (v: string | null) => void
}) {
...
export default function WriteEditorPage() {
  return (
    <Suspense
      fallback={
        <div className="flex items-center justify-center gap-3 min-h-screen text-sm text-muted-foreground">
          <LoadingDots size="md" className="text-primary" />
          <span>Yuklanmoqda...</span>
        </div>
      }
    >
      <WriteEditorPageInner />
    </Suspense>
  )
}

function WriteEditorPageInner() {
  const { state } = useAuth()
  ...
  const [customSlug, setCustomSlug] = useState<string | null>(null) // null = auto (title-dan)
  ...
  useEffect(() => {
    ...
    postsApi.myGet(token, editUuid).then((post) => {
      if (cancelled) return
      setTitle(post.title ?? "")
      setCustomSlug(post.slug ?? null) // mavjud slug ni yuklaymiz
  ...
  const buildPostData = useCallback(() => ({
    ...
    slug: customSlug?.trim() || undefined, // null/bo'sh = auto (backend title'dan hosil qiladi)
    ...
    is_pinned: isPinned, scheduled_at: localDatetimeToIso(scheduledAt),
  }), [title, content, excerpt, cover, customSlug, visibility, selectedCategories,
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      if (published?.id) {
        router.push(localPostHref(published))
      } else {
      ...
      {sidebarOpen && (
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <SidebarPanel
            ...
            customSlug={customSlug} onSlug={setCustomSlug}
          />
        </div>
      )}
