...
// Telegram endi bu ro'yxatda emas — u alohida accordion sifatida render qilinadi
const utilityItems = [
...
// ─── Mobile Search Button (search bar is hidden below sm:, this is the fallback) ──
function MobileSearchButton() {
  ...
  if (open) {
    return (
      <div className="fixed inset-x-0 top-0 z-50 flex h-14 items-center gap-2 bg-white px-3" style={{ borderBottom: "1px solid var(--color-border-default)" }}>
        <button
          onClick={() => setOpen(false)}
          className="shrink-0 rounded-xl p-2 text-text-muted hover:bg-bg-muted"
          aria-label="Yopish"
        >
          <X size={18} />
        </button>
        <input
          ...
          className="flex-1 rounded-xl bg-bg-muted px-3 py-2 text-sm outline-none"
        />
      </div>
    )
  }
  ...
  return (
    <button
      ...
      className="flex h-9 w-9 items-center justify-center rounded-xl text-text-muted transition-colors hover:bg-bg-muted sm:hidden"
      aria-label="Qidirish"
    >
      <Search size={17} />
    </button>
...
// ─── Telegram Accordion ─────────────────────────────────────────────────────
// Bosilganda o'z tagida sub-menyularni (Akkaunt / Kanallar / Tasdiqlash)
// pastga ochadi — alohida sahifaga o'tmaydi.
function TelegramAccordion({
  pathname,
  collapsed,
  onNavigate,
}: {
  ...
  const isTelegramActive = pathname === "/telegram" || pathname.startsWith("/telegram/")
  const [open, setOpen] = useState(isTelegramActive)

  // Sub-sahifaga to'g'ridan-to'g'ri kirilganda accordion avtomatik ochiq bo'lsin
  useEffect(() => {
    if (isTelegramActive) setOpen(true)
  }, [isTelegramActive])

  if (collapsed) {
    // Yig'ilgan sidebar holatida faqat ikonka — bosilsa birinchi sub-sahifaga o'tadi
    return (
      <Link
        href="/telegram/account"
        title="Telegram"
        onClick={onNavigate}
        className={cn(
          "group relative flex items-center justify-center rounded-xl px-0 py-2.5 text-sm font-medium transition-colors",
          isTelegramActive ? "text-primary" : "text-text-muted hover:text-text-primary",
        )}
      >
    ...
    <div>
      <button
        ...
        className={cn(
          ...
          isTelegramActive ? "text-primary" : "text-text-muted hover:bg-bg-muted hover:text-text-primary",
        )}
      >
        <BookOpen size={15} className="shrink-0" />
        <span className="flex-1 truncate text-left">Telegram</span>
        <ChevronRight
          size={13}
      ...
      {/* Sub-items — pastga ochiladi */}
      <div
        className="overflow-hidden transition-all duration-200"
        style={{ maxHeight: open ? telegramSubItems.length * 40 + 8 : 0 }}
      >
        <div className="ml-4 flex flex-col gap-0.5 border-l pl-3 pt-1" style={{ borderColor: "var(--color-border-default)" }}>
          {telegramSubItems.map(({ href, label }) => {
...
// ─── Main Layout ──────────────────────────────────────────────────────────────
// ─── SidebarInner ─────────────────────────────────────────────────────────────
// Render funksiyasi tashqarisida e'lon qilish:
//   • Har render'da qayta yaratilmaydi (referens barqaror)
//   • React ichki holati (focus, scroll) saqlanib qoladi
interface SidebarInnerProps {
  mobile?: boolean
  collapsed: boolean
  pathname: string
  user: import("@/types/api").UserMeResponse | null
  onNavigate?: () => void
  onLogout: () => void
}
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const isWriteRoute = pathname === "/write" || pathname.startsWith("/write/")

  // ── Auth guard ───────────────────────────────────────────────────────────────
  // MUHIM: navigatsiya/logout side-effect'lari faqat useEffect ichida chaqiriladi.
  // Ilgari bu kod render tanasida to'g'ridan-to'g'ri chaqirilardi — bu React'da
  // taqiqlangan holat va har bir re-render'da (masalan har 60 soniyada
  // fetchNotifCount natijasida) router.replace()/logout() qayta-qayta
  // chaqirilishiga, natijada ortiqcha so'rovlar va sahifa ochilmay qolishiga
  // sabab bo'lgan.
  const blockedHandledRef = useRef(false)

  useEffect(() => {
    if (isWriteRoute || loading) return
    ...
    if (user.status === "blocked" && !blockedHandledRef.current) {
      blockedHandledRef.current = true
      logout().finally(() => router.replace("/login?reason=blocked"))
    }
  }, [isWriteRoute, loading, user, pathname, router, logout])
  ...
  // Blocked accounts: logout is triggered from the effect above; just show a message.
  ...
  return (
    <div className="flex min-h-screen" style={{ background: "var(--color-bg-muted)" }}>
      ...
      >
        <SidebarInner collapsed={collapsed} pathname={pathname} user={user} onLogout={handleLogout} />

        {/*
          Yig'ish tugmasi — ILGARI "top-[4.5rem]" (72px) da edi, bu esa
          sahifa kontentining birinchi sarlavhasi bilan bir xil balandlikka
          to'g'ri kelib, unga yopishib/ustiga tushib qolardi (masalan
          "Profil sozlamalari" matnining "P" harfi ustida ko'rinardi).
          Endi tugma faqat logo qatori balandligida (h-14 = 56px, markazi
          ~24px) joylashadi — bu qat'iy ravishda sidebar sarlavhasi bilan
          bir qatorda, kontent hech qachon bu yerga yetib kelmaydi.
        */}
        <button
          onClick={() => setCollapsed((v) => !v)}
          className="absolute -right-3 top-4 z-10 flex h-6 w-6 items-center justify-center rounded-full bg-white transition-all duration-150 hover:border-primary hover:text-primary"
          style={{
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-40 lg:hidden">
          ...
          >
            <SidebarInner mobile collapsed={collapsed} pathname={pathname} user={user} onNavigate={() => setMobileOpen(false)} onLogout={handleLogout} />
          </aside>
        </div>
      )}
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        >
          ...
          <div className="flex items-center gap-2">
            {/* Search — hidden on mobile, mobile fallback button instead */}
