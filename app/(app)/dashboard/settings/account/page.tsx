...
type PendingAction = "delete-account" | "delete-other-sessions" | null
...
export default function AccountSettingsPage() {
  ...
  return (
    <DashboardContainer className="min-h-full py-4 sm:py-6 lg:py-8">
      {pendingAction === "delete-other-sessions" && (
        <ConfirmModal
          title="Boshqa qurilmalardan chiqish"
          description="Joriy qurilmadan tashqari barcha aktiv sessiyalar tugatiladi."
          confirmLabel={deletingOther ? "Chiqarilmoqda…" : "Chiqarish"}
          onConfirm={handleDeleteOtherSessions}
          onCancel={() => !deletingOther && setPendingAction(null)}
        />
      )}
      {pendingAction === "delete-account" && (
        <ConfirmModal
          title="Hisobni o'chirish"
          description="Hisobingiz, barcha maqolalaringiz, izohlaringiz va ma'lumotlaringiz butunlay o'chiriladi. Bu amalni qaytarib bo'lmaydi."
          confirmLabel={deleting ? "O'chirilmoqda…" : "Hisobni o'chirish"}
          onConfirm={handleDeleteAccount}
          onCancel={() => !deleting && setPendingAction(null)}
        />
      )}

      {/* Sahifa sarlavhasi */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-text-primary">Hisob sozlamalari</h1>
        <p className="mt-1 text-sm text-text-muted">Parolingizni, sessiyalaringizni va hisobingizni boshqaring.</p>
      </div>

      <div className="space-y-8">
        {/* Parolni o'zgartirish */}
        <section className="space-y-5 rounded-xl border border-border-default bg-white p-6">
        ...
        <section className="space-y-4 rounded-xl border border-border-default bg-white p-6">
          <div className="flex items-center justify-between">
            ...
            {sessions.length > 1 && (
              <button
                onClick={() => setPendingAction("delete-other-sessions")}
                disabled={deletingOther}
                className="rounded-xl px-3 py-1.5 text-xs font-medium text-red-500 hover:bg-red-50 hover:text-red-600 disabled:opacity-50"
              >
                {deletingOther ? <LoadingDots size="sm" /> : "Boshqalarini o'chirish"}
              </button>
            )}
          </div>
                ...
                >
                  ...
                  {!session.is_current && (
                    <button
                      ...
                      className="rounded-xl p-1.5 text-text-muted hover:bg-red-50 hover:text-red-500 transition-colors"
          ...
          <button
            onClick={logout}
            className="rounded-xl px-4 py-2 text-sm font-medium text-text-secondary transition-colors hover:bg-red-50 hover:text-red-500"
          >
        ...
        <section className="space-y-4 rounded-xl border border-red-200 bg-red-50/50 p-6">
          ...
          <button
            ...
            className="rounded-xl border border-red-200 px-4 py-2 text-sm font-medium text-red-500 transition-colors hover:bg-red-500 hover:text-white disabled:opacity-50"
          >
